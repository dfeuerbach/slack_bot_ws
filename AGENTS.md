...
- Prefer the instance helpers generated by `use SlackBot, otp_app: ...`:
  - `MyBot.push({method, body})` for Web API calls (`SlackBot.push(bot, ...)` still exists for dynamic instances).
...
- All outbound Web API calls should go through `MyBot.push/1` / `MyBot.push_async/1` (or the explicit `SlackBot.*(bot, ...)` forms) so both the per-channel rate limiter and the tier limiter can see the traffic.
...
- Respect `user_cache` TTL/cleanup values; do not mutate `%SlackBot.Config{}` structs in place to force refreshes. Use the cache helpers or explicit Slack API calls routed through `MyBot.push/1` (or `SlackBot.push(bot, ...)`).
